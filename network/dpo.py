...
class DPO(object):
    ...
    def __init__(self, config):
        ...
        #  state:
        self.state = tf.placeholder(tf.float32, [None, self.config['s_dim']], 'state')
        self.feed_dict['state'] = self.state
    
        #  actor
        self.ta_ph = tf.placeholder(tf.float32, [None, self.config['a_dim']], 'target_action')
        self.ad_ph = tf.placeholder(tf.float32, [None, 1], 'advantage')
        self.feed_dict['target_action'] = self.ta_ph
        self.feed_dict['advantage'] = self.ad_ph
        self.A = self._build_anet('action', trainable=True)

        with tf.variable_scope('loss'):
            self.aloss = tf.reduce_sum(self.ad_ph * tf.reduce_sum(tf.pow(self.A - self.ta_ph, 2), -1, True), name='a_loss')
